...
# when no covariate is included in the regression
...
data_simulate <- function(
  ...
  if (rule == 1) {
    Y_N <- exp(B_N_E_xi) * exp((phi + 1) * theta_i + tau_i)
    Y_T <- exp(B_T_E_xi) * exp(theta_i + tau_i)
    profit_N <- p * Y_N - C_N
    profit_T <- p * Y_T - C_T
    Y_N_post <- exp(B_N) * exp((phi + 1) * theta_i + tau_i + xi_N)
    Y_T_post <- exp(B_T) * exp(theta_i + tau_i + xi_T)
    h <- (profit_N >= profit_T)
  } else if (rule == 3) {
    Y_N <- exp(B_N_E_xi) * exp((phi + 1) * theta_i + tau_i)
    Y_T <- exp(B_T_E_xi) * exp(theta_i + tau_i)
    ...
    Y_T_post <- exp(B_T) * exp(theta_i + tau_i + xi_T)
    h <-  (
      (p * exp(B_N_E_xi) * exp((phi + 1) * theta_i) - C_N) >= 
      (p * exp(B_T_E_xi) * exp(theta_i) - C_T)
      )
  }
  ...
  return(list(y, profit, h, theta_i, profit_op, tau_i))
}
...
monte_carlo_simulation <- function(
  ...
  for (b in 1:numSim) {
    ...
    estimation <- structural_estimation(y, h)
...
monte_carlo_noinp_pm <- monte_carlo_simulation(
  param, numSim, rule = 1
  )
monte_carlo_noinp_pm[,7] %>% plot

# Figures when maximizing expected profits ===================
# return ------------
x_lim <- c(-0.5, 0.5)
noinp_pm_return_0 <- make_fig(monte_carlo_noinp_pm[,9], "Never adopters", x_lim)
noinp_pm_return_1 <- make_fig(monte_carlo_noinp_pm[,10], "Early adopters", x_lim)
noinp_pm_return_2 <- make_fig(monte_carlo_noinp_pm[,11], "Late adopters", x_lim)
noinp_pm_return_12 <- make_fig(monte_carlo_noinp_pm[,12], "Always adopters", x_lim)
ggsave(
  filename = file.path(
    git_dir,
    "Figures/noinp_pm_return.pdf"
  ),
  plot = grid.arrange(
    noinp_pm_return_0, noinp_pm_return_1, noinp_pm_return_2, noinp_pm_return_12, 
    ncol = 2
    ),
  height = 7,
  width = 7
...
# B and phi ---------
noinp_pm_B <- ggplot() + 
  geom_histogram(aes(x=monte_carlo_noinp_pm[,5]), binwidth=0.01) +
  coord_cartesian(xlim = c(0.8, 1.3)) +
  xlab("Estimated B") +
  ylab("Count") +
  geom_vline(xintercept = B, color = "red", linewidth = 1.0, alpha = 0.5) +
  theme_minimal()
noinp_pm_phi <- ggplot() +
  geom_histogram(aes(x=monte_carlo_noinp_pm[,6]), binwidth=0.01) + 
  coord_cartesian(xlim = c(-0.5, -0.2)) +
  xlab("Estimated phi") +
  ylab("Count") +
  geom_vline(xintercept = phi, color = "red", linewidth = 1.0, alpha = 0.5) +
  theme_minimal()
ggsave(
  filename = file.path(
    git_dir,
    "Figures/noinp_pm_B_phi.pdf"
  ),
  plot = grid.arrange(
    noinp_pm_B, noinp_pm_phi,
    ncol = 1
    ),
  height = 7,
  width = 7
...
monte_carlo_noinp_nopm <- monte_carlo_simulation(
  param, numSim, rule = 3
  )
monte_carlo_noinp_nopm[,7] %>% plot

# return ------------
x_lim <- c(-0.5, 0.5)
noinp_nopm_return_0 <- make_fig(monte_carlo_noinp_nopm[,9], "Never adopters", x_lim)
noinp_nopm_return_1 <- make_fig(monte_carlo_noinp_nopm[,10], "Early adopters", x_lim)
noinp_nopm_return_2 <- make_fig(monte_carlo_noinp_nopm[,11], "Late adopters", x_lim)
noinp_nopm_return_12 <- make_fig(monte_carlo_noinp_nopm[,12], "Always adopters", x_lim)
ggsave(
  filename = file.path(
    git_dir,
    "Figures/noinp_nopm_return.pdf"
  ),
  plot = grid.arrange(
    noinp_nopm_return_0,
    noinp_nopm_return_1,
    noinp_nopm_return_2,
    noinp_nopm_return_12,
    ncol=2
    ),
  height = 7,
  width = 7
...
# B and phi ---------
noinp_nopm_B <- ggplot() + 
  geom_histogram(aes(x=monte_carlo_noinp_nopm[,5]), binwidth=0.01) +
  coord_cartesian(xlim = c(0.8, 1.2)) +
  xlab("Estimated B") +
  ylab("Count") +
  geom_vline(xintercept = B, color = "red", linewidth = 1.0, alpha = 0.5) +
  theme_minimal()
noinp_nopm_phi <- ggplot() +
  geom_histogram(aes(x=monte_carlo_noinp_nopm[,6]), binwidth=0.01) + 
...
ggsave(
  filename = file.path(
    git_dir,
    "Figures/noinp_nopm_B_phi.pdf"
  ),
  plot = grid.arrange(
    noinp_nopm_B, noinp_nopm_phi,
    ncol = 1
    ),
  height = 7,
  width = 7
